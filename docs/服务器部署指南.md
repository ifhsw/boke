# 玄桥博客 — 服务器部署指南

## 前置条件

| 项目 | 要求 |
|------|------|
| 服务器 | Linux (Ubuntu 20.04+ / CentOS 7+) |
| 面板 | 宝塔面板 已安装 |
| Node.js | 18+ (推荐 20 LTS) |
| 数据库 | SQLite (无需额外安装) |
| 反向代理 | Nginx (宝塔自带) |
| 进程管理 | PM2 |
| 域名 | 已解析到服务器 IP |

---

## 第一步：服务器环境准备

### 1.1 宝塔面板安装 Node.js

```
宝塔面板 → 软件商店 → 搜索 "Node.js 版本管理器" → 安装
→ 安装 Node 20.x LTS
```

### 1.2 安装 PM2（全局）

```bash
npm install -g pm2
```

### 1.3 安装 Git（如果没有）

```bash
# Ubuntu/Debian
sudo apt update && sudo apt install git -y

# CentOS
sudo yum install git -y
```

---

## 第二步：上传代码到服务器

### 方式 A：Git 克隆（推荐）

```bash
# SSH 到服务器
cd /www/wwwroot

# 克隆仓库
git clone <你的仓库地址> blog

cd blog
```

### 方式 B：宝塔面板上传

```
宝塔面板 → 文件 → /www/wwwroot/ → 新建目录 blog
→ 将本地 blog 文件夹压缩为 .zip 上传
→ 解压到 /www/wwwroot/blog/
```

注意：**不要上传 `.env`、`prisma/dev.db`、`node_modules/`、`.next/`**

---

## 第三步：创建生产环境配置

### 3.1 创建 `.env` 文件

```bash
cd /www/wwwroot/blog
nano .env
```

内容：

```env
DATABASE_URL="file:./dev.db"
AUTH_SECRET="替换为随机密钥"
AUTH_URL="https://你的域名"
UPLOAD_DIR="./public/uploads"
```

### 3.2 生成 AUTH_SECRET

```bash
# 方式1：用 openssl 生成随机字符串
openssl rand -base64 32

# 方式2：用 node 生成
node -e "console.log(require('crypto').randomBytes(32).toString('base64'))"
```

把输出的字符串填入 `.env` 的 `AUTH_SECRET`。

### 3.3 创建 `ecosystem.config.cjs`（PM2 配置）

```bash
nano ecosystem.config.cjs
```

内容：

```js
module.exports = {
  apps: [
    {
      name: "blog",
      script: "node_modules/.bin/next",
      args: "start",
      cwd: "/www/wwwroot/blog",
      env: {
        NODE_ENV: "production",
        PORT: 3000,
      },
      instances: 1,
      exec_mode: "fork",
      autorestart: true,
      max_memory_restart: "500M",
    },
  ],
};
```

---

## 第四步：安装依赖 & 构建

```bash
cd /www/wwwroot/blog

# 1. 安装依赖
npm install

# 2. 初始化数据库（生成 dev.db 和表结构）
npx prisma migrate deploy

# 3. 导入初始数据（管理员账号 + 示例文章）
npx prisma db seed

# 4. 构建生产版本
npm run build
```

构建成功后输出类似：

```
✓ Generating static pages using 19 workers (14/14)
Route (app)
┌ ƒ /
├ ƒ /post/[slug]
...
```

---

## 第五步：启动应用（PM2）

```bash
# 启动
pm2 start ecosystem.config.cjs

# 查看状态
pm2 status

# 查看日志
pm2 logs blog

# 设置开机自启
pm2 save
pm2 startup
```

确认应用运行在 `http://localhost:3000`：

```bash
curl http://localhost:3000
```

---

## 第六步：配置 Nginx 反向代理

### 6.1 宝塔面板创建站点

```
宝塔面板 → 网站 → 添加站点
  - 域名：填入你的域名
  - 根目录：/www/wwwroot/blog/public
  - PHP 版本：纯静态
```

### 6.2 修改 Nginx 配置

```
宝塔面板 → 网站 → 点击域名 → 配置文件
```

替换为：

```nginx
server {
    listen 80;
    server_name 你的域名;

    # 上传的图片/文件
    location /uploads/ {
        alias /www/wwwroot/blog/public/uploads/;
        expires 30d;
        add_header Cache-Control "public, immutable";
    }

    # 静态资源缓存
    location /_next/static/ {
        alias /www/wwwroot/blog/.next/static/;
        expires 365d;
        add_header Cache-Control "public, immutable";
    }

    # 反向代理到 Next.js
    location / {
        proxy_pass http://127.0.0.1:3000;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection "upgrade";
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
        proxy_cache_bypass $http_upgrade;
    }
}
```

保存后重载 Nginx。

---

## 第七步：配置 SSL（HTTPS）

```
宝塔面板 → 网站 → 点击域名 → SSL
→ Let's Encrypt → 申请证书 → 勾选 "强制 HTTPS"
```

---

## 第八步：防火墙 & 安全

### 8.1 宝塔防火墙

```
宝塔面板 → 安全 → 防火墙
  - 放行端口：80 (HTTP)、443 (HTTPS)
  - 不放行端口：3000（Next.js 只监听本地）
```

### 8.2 文件权限

```bash
chmod 755 /www/wwwroot/blog
chmod 644 /www/wwwroot/blog/.env
chmod 644 /www/wwwroot/blog/prisma/dev.db
chmod -R 755 /www/wwwroot/blog/public/uploads
```

---

## 第九步：数据库备份（宝塔计划任务）

```
宝塔面板 → 计划任务 → 添加
  - 任务名称：博客数据库备份
  - 执行周期：每天 03:00
  - 脚本内容：
```

```bash
#!/bin/bash
SRC="/www/wwwroot/blog/prisma/dev.db"
DST="/www/backup/blog-$(date +%Y%m%d).db"
cp "$SRC" "$DST"
# 保留最近 30 天的备份
find /www/backup/ -name "blog-*.db" -mtime +30 -delete
```

---

## 第十步：更新部署（后续）

### 代码有更新时

```bash
cd /www/wwwroot/blog

# 拉取最新代码
git pull

# 安装新依赖（如果有）
npm install

# 重新构建
npm run build

# 重启应用（零停机）
pm2 reload blog
```

---

## 默认管理员账号

| 字段 | 值 |
|------|-----|
| 邮箱 | `admin@blog.com` |
| 密码 | `admin123` |

**上线后请立即修改密码！**

可通过以下方式：
1. 访问 `https://你的域名/login` 登录
2. 或修改 `prisma/seed.ts` 中的默认密码后重新 seed

---

## 故障排查

### 502 Bad Gateway

```bash
# 检查 Next.js 是否在运行
pm2 status

# 查看错误日志
pm2 logs blog --err

# 检查端口占用
netstat -tlnp | grep 3000
```

### 页面无法加载 / API 报错

```bash
# 确认 .env 中的 AUTH_URL 为 https://你的域名
cat /www/wwwroot/blog/.env

# 重启应用
pm2 restart blog
```

### 数据库被锁定

```bash
# SQLite 单写锁问题，重启应用即可
pm2 restart blog
```

### 上传图片失败

```bash
# 确认 uploads 目录存在且可写
ls -la /www/wwwroot/blog/public/uploads/
chmod 755 /www/wwwroot/blog/public/uploads/
```

---

## 目录结构（服务器上）

```
/www/wwwroot/blog/
├── .env                  # 环境变量（敏感，不要提交 git）
├── .next/                # 构建产物
├── ecosystem.config.cjs  # PM2 配置
├── node_modules/         # 依赖
├── package.json
├── prisma/
│   ├── dev.db            # SQLite 数据库文件（需备份）
│   ├── schema.prisma
│   └── seed.ts
├── public/
│   └── uploads/          # 上传的图片
├── src/                  # 源代码
└── next.config.ts
```
