...
个人博客，支持混合型内容（技术、随笔等），温暖文艺风格。管理员通过后台管理内容，读者可注册登录、写文章（草稿需管理员审核发布）、评论留言。支持用户个人主页和资料编辑。部署在自有 Linux 服务器（宝塔面板）。
...
| 账号管理 | `/account`        | 用户自助：编辑个人资料、修改密码、管理自己的提问 |
| 我的文章 | `/my-posts`       | 用户自己的文章列表，可新建/编辑/删除草稿     |
| 用户主页 | `/user/[username]`| 公开用户主页，展示个人信息和已发布文章       |
| 登录   | `/login`          | 读者和管理员登录                 |
| 注册   | `/register`       | 读者注册                     |
...
- 写文章（自动保存为草稿，管理员审核后发布）
- 管理自己的文章（编辑、删除，在 `/my-posts` 集中管理）
- 编辑个人资料（简介、位置、网站、GitHub/Twitter）
- 修改自己的密码（需验证当前密码）
- 查看和删除自己的匿名提问
- 在 `/account` 账号管理页集中管理
...
- 支持多级嵌套回复（递归树形结构）
- 回复时显示 "回复 @用户名" 提示
- 用户名可点击跳转到用户主页 `/user/[username]`
- 管理员可删除任意评论

### 用户主页

- `/user/[username]` 公开可见
- 展示：头像、角色标签、个人简介、位置、网站/社交链接
- 展示该用户的已发布文章列表
- 显示文章数、评论数、注册时间等统计
